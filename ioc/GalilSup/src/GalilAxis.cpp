...
asynStatus GalilAxis::setDefaults(int limit_as_home, char *enables_string, int switch_type)
{
	...
	switch_as_home_ = limit_as_home_;  //LIU 2016
			     ...
			     char axis_thread_code[])
{
	...
	// LIU 2016
//	hsw use to switch between  "home switch" and Encoder home index 
//	sprintf(axis_thread_code,"%sIF ((hjog%c=0) & (hsw%c=1))\nhjog%c=1;FE%c;WT50;BG%c\nENDIF\n",axis_thread_code,c,c,c,c,c); //Remove the "FE"
	sprintf(axis_thread_code,"%sIF ((hjog%c=0) & (hsw%c=1))\nhjog%c=1;WT50;BG%c\nENDIF\n",axis_thread_code,c,c,c,c);
	sprintf(axis_thread_code,"%sIF ((hjog%c=0) & (hsw%c=0) & (ueip%c=1))\nhjog%c=1;FI%c;WT50;BG%c\nENDIF\n",axis_thread_code,c,c,c,c,c,c);

     	sprintf(axis_thread_code,"%sIF ((hjog%c=1) & (hsw%c=1) & (_HM%c=0))\nST%c;WT50;hjog%c=2\nENDIF\n",axis_thread_code,c,c,c,c,c);	
 	sprintf(axis_thread_code,"%sIF ((hjog%c=1) & (hsw%c=0) & (ueip%c=1) & (_BG%c=0))\nhjog%c=3\nENDIF\n",axis_thread_code,c,c,c,c,c);
		
	sprintf(axis_thread_code,"%sIF ((hjog%c=2) | (hjog%c=3))\nhjog%c=0;home%c=0;homed%c=1;WT50;MG \"homed%c\",homed%c;MG \"home%c\",home%c\nENDIF\n",axis_thread_code,c,c,c,c,c,c,c,c,c);
	sprintf(axis_thread_code,"%sIF ((hjog%c=1) & ((_LR%c=0) | (_LF%c=0)))\nhome%c=0;hjog%c=0\nENDIF\n",axis_thread_code,c,c,c,c,c);
...
// LIU end	
	...
	/*initialise home switch variable*/ // LIU 2016
	sprintf(pC_->cmd_, "hsw%c=1", c);
	pC_->sync_writeReadController();
...
asynStatus GalilAxis::setupHome(double maxVelocity, int forwards)
{
   ...
   pC_->sync_writeReadController();
      ...
      //Set hjog status to false   //LIU 2015-08
	sprintf(pC_->cmd_, "hjog%c=0", axisName_);
	pC_->sync_writeReadController();
	if(switch_as_home_)
	{
		sprintf(pC_->cmd_, "hsw%c=1", axisName_);
		pC_->sync_writeReadController();
	}
	else
	{ 
		sprintf(pC_->cmd_, "hsw%c=0", axisName_);
		pC_->sync_writeReadController();
	}
  ...
  while (true)
     ...
     switch (request)
        ...
        case MOTOR_POST: if (pC_->getStringParam(axisNo_, pC_->GalilPost_, (int)sizeof(post), post) == asynSuccess)
			    ...
			    sprintf(pC_->cmd_, "%s%c", post, axisName_);   //jwh
...
void GalilAxis::executePrem(void)
{
  ...
  if (pC_->getStringParam(axisNo_, pC_->GalilPrem_, (int)sizeof(prem), prem) == asynSuccess)
	...
	sprintf(pC_->cmd_, "%s%c", prem, axisName_);   //jwh
        cout << "write prem:" << pC_->cmd_<<"\n";  //jwh
